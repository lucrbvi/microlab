### Minibook
This is a personal notebook to learn and experiment theories in the deep-learning space from math to code. Everything is designed to be simple enough to be easily understandable with basic knowledge in linear algebra and Python.
Also all the code is written in "pure" Python using [tinygrad](https://tinygrad.org/) and numpy.
### Prerequisites
* Good understanding of [linear algebra](https://en.wikipedia.org/wiki/Linear_algebra) (matrices, derivative, vectors)
* Understand basics of [Python](https://www.python.org/)and programming
### Get started
I recommend you to [clone the repository](https://git-scm.com/docs/git-clone) and open the vault in Obsidian. You can still read it all through the GitHub Web frontend but it's a far worse experience.
Then you should install the packages (listed in `pyproject.toml`) and your good to go! 
I've also written a single line bash script to launch the virtual environment easily.
### Run scripts
```sh
python3 -m scripts.attention
python3 -m scripts.experimentation.attention
```
